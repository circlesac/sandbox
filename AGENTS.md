...
hardware-derived `sandbox-<DEVICE_HASH>.crcl.es` hostname. Do not replace it
with a shared name such as `sandbox-local.crcl.es` or cgrok's random hostname:
the stable per-device name prevents collisions when more than one machine runs
the control plane.
